//页面初始化,显示最新一年全国的数据

$(function () {
    //初始化方法
    getYearAll("cmb_year");
    getProvinceAll("cmb_province");
    $("#div_title_org").html($("#cmb_province").val()+"机构汇总");
    $("#div_title_emp").html($("#cmb_province").val()+"人员汇总");
    getGOVandHRInfo($("#cmb_year").val(),$("#cmb_province").val());

});
//数据查询-更新
function queryData(){

    $("#div_title_org").html($("#cmb_province").val()+"机构汇总");
    $("#div_title_emp").html($("#cmb_province").val()+"人员汇总");
    getGOVandHRInfo($("#cmb_year").val(),$("#cmb_province").val());
}
//获取所有的年份
function getYearAll(domId){
    $.ajax({
        type:"POST",
        url:"/basis/getYearAllForORGandHR.action",
        async:false,
        error:function(data){
            return;
        },
        success:function(data){
            $("#"+domId).empty();
            if(data.length > 0){
                var items = data.replace("[","").replace("]","").replace(/\"/g,"").split(",");  //将JAVA中的List<String>转为JS中的数组
                for (var i = 0; i < items.length; i++ ){
                    var option = "<option value=\"" + items[i].replace("\"","") + "\">" + items[i]+ "</option>";
                    $("#"+domId).append(option);
                }
                $("#"+domId).selectpicker('refresh');

            }
        }
    });
}
//获取所有的省份（包括全国）
function getProvinceAll(domId){
    $.ajax({
        type:"POST",
        url:"/basis/getProvinceAllForORGandHR.action",
        async:false,
        error:function(data){
            return;
        },
        success:function(data){
            $("#"+domId).empty();
            if(data.length > 0){
                var items = data.replace("[","").replace("]","").replace(/\"/g,"").split(",");  //将JAVA中的List<String>转为JS中的数组
                for (var i = 0; i < items.length; i++ ){
                    var option = "<option value=\"" + items[i].replace("\"","") + "\">" + items[i]+ "</option>";
                    $("#"+domId).append(option);
                }
                $("#"+domId).selectpicker('refresh');
                $("#"+domId).value="全国总计";
            }
        }
    });
}

function getGOVandHRInfo(year_val, province_val){
    $.ajax({
        type:"POST",
        url:"/basis/getGOVandProvinceInfo.action",
        async:true,
        data:{"year":year_val,"province":province_val},
        error:function(data){
            return;
        },
        success:function(data) {
            var jdata = JSON.parse(data);

            $("#div_admin_bzinfo").html("编制： "+jdata["Emp_BZ_Count"]+"， 在职： "+jdata["Emp_ZZ_Count"]);
            $("#div_company_bzinfo").html("编制： "+jdata["Emp_SY_BZ_Count"]+"， 在职： "+jdata["Emp_SY_ZZ_Count"]);
            var option_org = {
                title: {
                    x: 'center'
                },
                tooltip: {
                    trigger: 'item',
                    formatter: "{a} <br/>{b} : {c} ({d}%)"
                },
                legend: {
                    orient: 'vertical',
                    left: 'left',
                    data: ['行政机构', '事业机构'],
                    textStyle:{
                        color:'#fff'
                    }
                },
                color: ['#8bc34a', '#ffb300'],
                series: [{
                    name: '机构数量',
                    type: 'pie',
                    radius: '85%',
                    center: ['40%', '50%'],
                    data: [{
                        value: jdata["ORG_ADMIN"],
                        name: '行政机构'
                    },
                        {
                            value: jdata["ORG_NONADMIN"],
                            name: '事业机构'
                        },
                    ],
                    itemStyle: {
                        emphasis: {
                            shadowBlur: 10,
                            shadowOffsetX: 0,
                            shadowColor: 'rgba(0, 0, 0, 0.5)'
                        },
                    },
                    label: {
                        normal:{
                            position: 'inside',
                            formatter: '{c}个',
                            textStyle:{
                                fontSize:20,
                            },
                            color:'#fff'
                        }
                    }
                }]
            };
            //人员数据汇总
            var option_hr_cnt = {
                title: {
                    x: 'center',
                },
                tooltip: {
                    trigger: 'item',
                    formatter: "{a} <br/>{b} : {c} ({d}%)"
                },
                legend: {
                    orient: 'vertical',
                    left: 'left',
                    data: ['行政人员', '事业人员','离退休人员','其他从业人员'],
                    textStyle:{
                        color:'#fff'
                    }
                },
                color: ['#8bc34a', '#ffb300','#00897b','#d81b60'],
                series: [{
                    name: '人员数量',
                    type: 'pie',
                    radius: '85%',
                    center: ['65%', '50%'],
                    data: [{
                        value: jdata['Emp_XZ_ZZ_Count'],
                        name: '行政人员'
                    },
                        {
                            value: jdata['Emp_SY_ZZ_Count'],
                            name: '事业人员'
                        },
                        {
                            value: jdata['Emp_Retired'],
                            name: '离退休人员'
                        },
                        {
                            value: jdata['Emp_Other'],
                            name: '其他从业人员'
                        }
                    ],
                    itemStyle: {
                        emphasis: {
                            shadowBlur: 10,
                            shadowOffsetX: 0,
                            shadowColor: 'rgba(0, 0, 0, 0.5)'
                        },
                    },
                    label: {
                        normal:{
                            position: 'inside',
                            formatter: '{c}人',
                            textStyle:{
                                fontSize:13,
                            },
                            color:'#fff'
                        }
                    }
                }]
            };
            //行政编制信息
            var option_admin_bz = {
                title: {
                    text: '',
                    left: 'center'
                },
                color: ['#61A8FF'], //进度条颜色
                /* grid: {
                     left: '-10%',  //如果离左边太远就用这个......
                     //right: '14%',
                     bottom: '5%',
                     top: '5%',
                     containLabel: true
                 },*/
                xAxis: [{
                    show: false,
                },
                    //由于下边X轴已经是百分比刻度了,所以需要在顶部加一个X轴,刻度是金额,也隐藏掉
                    {
                        show: false,
                    }
                ],
                yAxis: {
                    type: 'category',
                    axisLabel: {
                        show: false, //让Y轴数据不显示
                    },
                    itemStyle: {

                    },
                    axisTick: {
                        show: false, //隐藏Y轴刻度
                    },
                    axisLine: {
                        show: false, //隐藏Y轴线段
                    },
                    data: [],
                },
                series: [
                    //背景色--------------------我是分割线君------------------------------//
                    {
                        show: true,
                        type: 'bar',
                        barGap: '-100%',
                        barWidth: '25%', //统计条宽度
                        itemStyle: {
                            normal: {
                                barBorderRadius: 15,
                                color: 'rgba(227,182,31,0.2)'
                            },
                        },
                        z:1,
                        data: [100],
                    },
                    //蓝条--------------------我是分割线君------------------------------//
                    {
                        show: true,
                        type: 'bar',
                        barGap: '-100%',
                        barWidth: '25%', //统计条宽度
                        itemStyle: {
                            normal: {
                                barBorderRadius: 20, //统计条弧度
                                color:'#e3b61f'
                            },
                        },
                        max: 1,
                        label: {
                            normal: {
                                show: true,
                                textStyle: {
                                    color: '#fff', //百分比颜色
                                },
                                position: 'inside',
                                //百分比格式
                                formatter: "{c}%"
                            }
                        },
                        labelLine: {
                            show: false,
                        },
                        z:2,
                        data: [((jdata['Emp_XZ_ZZ_Count']/jdata['Emp_XZ_BZ_Count'])*100).toFixed(4)],
                    },
                ]
            };
            //行政按教育分
            var option_admin_edu={
                "title": {
                    "text": "",
                    "left": "center",
                    "y": "10",
                    "textStyle": {
                        "color": "#fff"
                    }
                },

                "grid": {
                    "left": "12%",
                    "top": "10%",
                    "bottom": 10,

                    "height":200,
                },
                "tooltip": {
                    "trigger": "item",
                    "textStyle": {
                        "fontSize": 12
                    },
                    "formatter": "{b0}:{c0}"
                },
                "xAxis": {
                    "max": jdata["Emp_XZ_WH_BK"],

                    "splitLine": {
                        "show": false
                    },
                    "axisLine": {
                        "show": false
                    },
                    "axisLabel": {
                        "show": false
                    },
                    "axisTick": {
                        "show": false
                    }
                },
                "yAxis": [
                    {
                        "type": "category",
                        "inverse": true,
                        "data": [
                            "博士研究生",
                            "硕士研究生",
                            "大学本科",
                            "大学专科",
                            "高中及以下"

                        ],
                        "axisLine": {
                            "show": false
                        },
                        "axisTick": {
                            "show": false
                        },
                        "axisLabel": {
                            "margin": -4,
                            "textStyle": {
                                "color": "#fff",
                                "fontSize": 16.25
                            }
                        }
                    },

                ],
                "series": [
                    {
                        "type": "pictorialBar",
                        "symbol": "image://data:image/png;base64,",
                        "symbolRepeat": "fixed",
                        "symbolMargin": "5%",
                        "symbolClip": true,
                        "symbolSize": 22.5,
                        "symbolPosition": "start",
                        "symbolOffset": [
                            20,
                            0
                        ],

                        "data": [
                            jdata["Emp_XZ_WH_BS"],
                            jdata["Emp_XZ_WH_SS"],
                            jdata["Emp_XZ_WH_BK"],
                            jdata["Emp_XZ_WH_ZK"],
                            jdata["Emp_XZ_WH_GZ"]
                        ],
                        "z": 10
                    },
                    {
                        "type": "pictorialBar",
                        "itemStyle": {
                            "normal": {
                                "opacity": 0.3
                            }
                        },
                        "label": {
                            "normal": {
                                "show": false
                            }
                        },
                        "animationDuration": 0,
                        "symbolRepeat": "fixed",
                        "symbolMargin": "5%",
                        "symbol": "image://data:image/png;base64,",
                        "symbolSize": 22.5,

                        "symbolPosition": "start",
                        "symbolOffset": [
                            20,
                            0
                        ],
                        "data": [
                            jdata["Emp_XZ_WH_BS"],
                            jdata["Emp_XZ_WH_SS"],
                            jdata["Emp_XZ_WH_BK"],
                            jdata["Emp_XZ_WH_ZK"],
                            jdata["Emp_XZ_WH_GZ"]
                        ],
                        "z": 5
                    }
                ]

            };
            //行政工作性质
            var option_admin_xz = {
                legend: {
                    orient: 'vertical',
                    x: 'left',
                    data:['公务员','工勤人员'],
                    show:false
                },
                color:['#ffb300','#94d96c'],
                series: [
                    {
                        name:'按工作性质分',
                        type:'pie',
                        radius: ['55%', '75%'],
                        label: {
                            normal: {
                                formatter: ' {b|{b}：}{c}  {per|{d}%}  ',
                                backgroundColor: 'rgb(30,46,63)',
                                borderColor: '#aaa',
                                borderWidth: 1,
                                borderRadius: 4,
                                rich: {
                                    a: {
                                        color: '#999',
                                        lineHeight: 26,
                                        align: 'center'
                                    },
                                    hr: {
                                        borderColor: '#aaa',
                                        width: '100%',
                                        borderWidth: 0.5,
                                        height: 0
                                    },
                                    b: {
                                        fontSize: 14,
                                        lineHeight: 36
                                    },
                                    per: {
                                        color: '#eee',
                                        backgroundColor: '#334455',
                                        padding: [2, 4],
                                        borderRadius: 2
                                    }
                                }
                            },
                            emphasis: {
                                show: true,
                                textStyle: {
                                    fontSize: '16',
                                    fontWeight: 'bold'
                                }
                            }
                        },
                        data:[
                            {value:jdata['Emp_XZ_XZ_GWY'], name:'公务员'},
                            {value:jdata['Emp_XZ_XZ_AQRY'], name:'工勤人员'}
                        ]
                    }
                ]
            };
            var org_all_chart = echarts.init(document.getElementById("div_org_cnt_all"));
            org_all_chart.setOption(option_org);
            //var org_province_chart = echarts.init(document.getElementById("div_org_cnt_province"));
            //org_province_chart.setOption(option_province);
            var hr_all_chart = echarts.init(document.getElementById("div_hr_all"));
            hr_all_chart.setOption(option_hr_cnt);
            var admin_edu_chart = echarts.init(document.getElementById("div_admin_edu"));
            admin_edu_chart.setOption(option_admin_edu);
            var admin_bz_chart = echarts.init(document.getElementById("div_admin_bz"));
            admin_bz_chart.setOption(option_admin_bz);
            var admin_xz_chart = echarts.init(document.getElementById("div_admin_xz"));
            admin_xz_chart.setOption(option_admin_xz);

            //事业人员信息
            var option_company_bz = {
                title: {
                    text: '',
                    left: 'center'
                },
                color: ['#61A8FF'], //进度条颜色
                /* grid: {
                     left: '-10%',  //如果离左边太远就用这个......
                     //right: '14%',
                     bottom: '5%',
                     top: '5%',
                     containLabel: true
                 },*/
                xAxis: [{
                    show: false,
                },
                    //由于下边X轴已经是百分比刻度了,所以需要在顶部加一个X轴,刻度是金额,也隐藏掉
                    {
                        show: false,
                    }
                ],
                yAxis: {
                    type: 'category',
                    axisLabel: {
                        show: false, //让Y轴数据不显示
                    },
                    itemStyle: {

                    },
                    axisTick: {
                        show: false, //隐藏Y轴刻度
                    },
                    axisLine: {
                        show: false, //隐藏Y轴线段
                    },
                    data: [],
                },
                series: [
                    //背景色--------------------我是分割线君------------------------------//
                    {
                        show: true,
                        type: 'bar',
                        barGap: '-100%',
                        barWidth: '35%', //统计条宽度
                        itemStyle: {
                            normal: {
                                barBorderRadius: 15,
                                color: 'rgba(227,182,31,0.2)'
                            },
                        },
                        z:1,
                        data: [100],
                    },
                    //蓝条--------------------我是分割线君------------------------------//
                    {
                        show: true,
                        type: 'bar',
                        barGap: '-100%',
                        barWidth: '35%', //统计条宽度
                        itemStyle: {
                            normal: {
                                barBorderRadius: 20, //统计条弧度
                                color:'#e3b61f'
                            },
                        },
                        max: 1,
                        label: {
                            normal: {
                                show: true,
                                textStyle: {
                                    color: '#fff', //百分比颜色
                                },
                                position: 'inside',
                                //百分比格式
                                formatter: "{c}%"
                            }
                        },
                        labelLine: {
                            show: false,
                        },
                        z:2,
                        data: [((jdata["Emp_SY_ZZ_Count"]/jdata["Emp_SY_BZ_Count"])*100).toFixed(2)],
                    },

                ]
            };
            var option_company_edu={
                "title": {
                    "text": "",
                    "left": "center",
                    "y": "10",
                    "textStyle": {
                        "color": "#fff"
                    }
                },

                "grid": {
                    "left": "12%",
                    "top": "10%",
                    "bottom": 10,

                    "height":200,
                },
                "tooltip": {
                    "trigger": "item",
                    "textStyle": {
                        "fontSize": 12
                    },
                    "formatter": "{b0}:{c0}"
                },
                "xAxis": {
                    "max": jdata["Emp_SY_WH_BK"],

                    "splitLine": {
                        "show": false
                    },
                    "axisLine": {
                        "show": false
                    },
                    "axisLabel": {
                        "show": false
                    },
                    "axisTick": {
                        "show": false
                    }
                },
                "yAxis": [
                    {
                        "type": "category",
                        "inverse": true,
                        "data": [
                            "博士研究生",
                            "硕士研究生",
                            "大学本科",
                            "大学专科",
                            "高中及以下"

                        ],
                        "axisLine": {
                            "show": false
                        },
                        "axisTick": {
                            "show": false
                        },
                        "axisLabel": {
                            "margin": -4,
                            "textStyle": {
                                "color": "#fff",
                                "fontSize": 16.25
                            }
                        }
                    },

                ],
                "series": [
                    {
                        "type": "pictorialBar",
                        "symbol": "image://data:image/png;base64,",
                        "symbolRepeat": "fixed",
                        "symbolMargin": "5%",
                        "symbolClip": true,
                        "symbolSize": 22.5,
                        "symbolPosition": "start",
                        "symbolOffset": [
                            20,
                            0
                        ],

                        "data": [
                            jdata["Emp_SY_WH_BS"],
                            jdata["Emp_SY_WH_SS"],
                            jdata["Emp_SY_WH_BK"],
                            jdata["Emp_SY_WH_ZK"],
                            jdata["Emp_SY_WH_GZ"]

                        ],
                        "z": 10
                    },
                    {
                        "type": "pictorialBar",
                        "itemStyle": {
                            "normal": {
                                "opacity": 0.3
                            }
                        },
                        "label": {
                            "normal": {
                                "show": false
                            }
                        },
                        "animationDuration": 0,
                        "symbolRepeat": "fixed",
                        "symbolMargin": "5%",
                        "symbol": "image://data:image/png;base64,",
                        "symbolSize": 22.5,

                        "symbolPosition": "start",
                        "symbolOffset": [
                            20,
                            0
                        ],
                        "data": [
                            102,
                            7534,
                            105343,
                            60798,
                            11431
                        ],
                        "z": 5
                    }
                ]

            };
            //按工作性质分
            var option_company_gzxz = {

                title: {
                    text: '按工作性质分',
                    textStyle: {
                        color: '#f2f2f2',
                        fontSize: 16,
                        // align: 'center'
                    },
                    // top: "5%",
                    left: 120
                },
                legend: {
                    top: "12%",
                    left: 10,
                    textStyle: {
                        color: '#f2f2f2',
                        fontSize: 14,
                    },
                    icon: 'circle',
                    data: ['专业技术人员','管理员人','工人和工勤人员'],
                    orient:'vertical'
                },
                grid:{
                    top:50,
                },
                series: [
                    // 主要展示层的
                    {
                        radius: ['43%', '71%'],
                        center: ['55%', '60%'],
                        type: 'pie',
                        label: {
                            normal: {
                                formatter:'{c}人 \r\n {d}%',
                                position:'inside',
                                show: true,
                                textStyle:{
                                    fontSize:14
                                }
                            },
                            emphasis: {
                                formatter:'{c}人 \n {d}%',
                                position:'inside',
                                show: true,
                                textStyle:{
                                    fontSize:16
                                }
                            }
                            ,
                        },

                        labelLine: {
                            normal: {
                                show: false
                            },
                            emphasis: {
                                show: false
                            }
                        },
                        name: "专业技术",
                        data: [{
                            value: jdata['Emp_SY_XZ_ZYJS'],
                            name: '专业技术人员'
                        },
                            {
                                value: jdata['Emp_SY_XZ_GL'],
                                name: '管理人员'
                            },
                            {
                                value: jdata['Emp_SY_XZ_GR'],
                                name: '工人和工勤人员'
                            }
                        ],
                        tooltip: {
                            formatter: "{a}：<br/>{b}: {c}人"
                        }
                    }
                ]
            };
            //按技术职称分
            var option_company_jszc = {

                title: {
                    text: '按技术职称分',
                    textStyle: {
                        color: '#f2f2f2',
                        fontSize: 16,
                        // align: 'center'
                    },
                    // top: "5%",
                    left: 120
                },
                legend: {
                    top: "12%",
                    left: 10,
                    textStyle: {
                        color: '#f2f2f2',
                        fontSize: 14,
                    },
                    icon: 'circle',
                    data:['初级和未评定','中级','高级'],
                    orient:'vertical'
                },
                grid:{
                    top:50,
                },
                series: [
                    // 主要展示层的
                    {
                        radius: ['43%', '71%'],
                        center: ['55%', '60%'],
                        type: 'pie',
                        label: {
                            normal: {
                                formatter:'{c}人 \r\n {d}%',
                                position:'inside',
                                show: true,
                                textStyle:{
                                    fontSize:14
                                }
                            },
                            emphasis: {
                                formatter:'{c}人 \n {d}%',
                                position:'inside',
                                show: true,
                                textStyle:{
                                    fontSize:16
                                }
                            }
                            ,
                        },

                        labelLine: {
                            normal: {
                                show: false
                            },
                            emphasis: {
                                show: false
                            }
                        },
                        name: "专业技术",
                        data:  [{
                            value: jdata['Emp_SY_ZC_GJ'],
                            name: '高级'
                        },
                            {
                                value: jdata['Emp_SY_ZC_ZJ'],
                                name: '中级'
                            },
                            {
                                value: jdata['Emp_SY_ZC_CJ'],
                                name: '初级和未评定'
                            }
                        ],
                        tooltip: {
                            formatter: "{a}：<br/>{b}: {c}人"
                        }
                    }
                ]
            };
             var company_edu_chart = echarts.init(document.getElementById("div_company_edu"));
             company_edu_chart.setOption(option_company_edu);
             var company_bz_chart = echarts.init(document.getElementById("div_company_bz"));
             company_bz_chart.setOption(option_company_bz);
             var company_xz_chart = echarts.init(document.getElementById("div_company_xz"));
             company_xz_chart.setOption(option_company_gzxz);
             var company_zc_chart = echarts.init(document.getElementById("div_company_zc"));
             company_zc_chart.setOption(option_company_jszc);
        }
    });
}



var spirit = 'image://data:image/png;base64,';



/*事业编制人员按性质分-begin*/
function deepCopy(obj) {
    if (typeof obj !== 'object') {
        return obj;
    }
    var newobj = {};
    for (var attr in obj) {
        newobj[attr] = obj[attr];
    }
    return newobj;
}
var xData = [],
    yData = [];
var data = [{
    value: 69130,
    name: '专业技术人员'
},
    {
        value: 32933,
        name: '管理人员'
    },
    {
        value: 30025,
        name: '工人和工勤人员'
    }
];
data.map((a, b) => {
    xData.push(a.name);
yData.push(a.value);
});
var startColor = ['#7a18ed', '#00bbce', '#ea865a'];
var endColor = ['#2743ed', '#00c4a5', '#ea2e41'];
var borderStartColor = ['#ee36ff', '#05fcfb', '#ffa597'];
var borderEndColor = ['#8171ff', '#05ffff', '#ff6584'];
var RealData = [];
var borderData = [];
data.map((item, index) => {
    var newobj = deepCopy(item);
var newobj1 = deepCopy(item);
RealData.push(newobj);
borderData.push(newobj1);
});
RealData.map((item, index) => {
    item.itemStyle = {
    normal: {
        color: {
            type: 'linear',
            x: 0,
            y: 0,
            x2: 0,
            y2: 1,
            colorStops: [{
                offset: 0,
                color: startColor[index] // 0% 处的颜色
            }, {
                offset: 1,
                color: endColor[index] // 100% 处的颜色
            }],
            globalCoord: false // 缺省为 false
        },
    }
}
});
borderData.map((item, index) => {
    item.itemStyle = {
    normal: {
        color: {
            type: 'linear',
            x: 0,
            y: 0,
            x2: 0,
            y2: 1,
            colorStops: [{
                offset: 0,
                color: borderStartColor[index] // 0% 处的颜色
            }, {
                offset: 1,
                color: borderEndColor[index] // 100% 处的颜色
            }],
            globalCoord: false // 缺省为 false
        },
    }
}
});

/*事业编制按性质分-end*/
